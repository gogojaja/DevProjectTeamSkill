...
> 沉淀自 2026-08-19 quiz-extraction-skill 评审能力增强（五视角评审 → 修订 → 实施），提炼为可复用的通用评审模式。适用任何需要「门禁 + 评审报告 + 可配置阈值」的验证场景。

### 6.1 评审四层模型

| 层 | 职责 | 判定主体 | 阻断性 |
|----|------|---------|--------|
| 结构层 | 字段存在/枚举/唯一性/引用完整性 | 脚本固化 | error |
| 语义层 | 答案一致性/干扰项有效性等判断 | AI 抽样判 | error/warning |
| 统计层 | 难度分布/覆盖度/分值合计 | 脚本统计 | warning |
| 决策层 | 各层结果聚合、签署、风险接受 | 聚合规则 | 门禁 |

### 6.2 关键模式

1. **抽样复审机制**：全量结构校验 + 抽样语义判断（缺省 `min(10, 题数×20%)`）。语义维度仅抽样，避免大语料全量 LLM 调用，报告须含 `seed` 保证可复现。
2. **阈值参数化**：所有规则阈值（相似度、覆盖白名单、抽样数）外置 config，不硬编码；`--strict` 开关决定 warning 是否阻断（保持既有行为兼容）。
3. **性能分桶**：O(n²) 两两比对用「长度分桶（±radius）」降阶，桶超上限（默认 50）降级跳过并告警，保证千级规模 ≤ 2s。
4. **报告脱敏**：评审报告内路径一律相对项目根，不含用户主目录前缀（分享/交付场景防目录结构泄露）。
5. **回归固化**：每个校验规则配套正反例测试 + golden fixture；既有通过样例（如现金管理 33 题）固化为回归基线，防止规则调整破坏既有产物。

### 6.3 落地建议

- 门禁（阻断）与评审报告（不阻断）分离为两步，评审层独立产出 `评审报告.json`（含维度状态/发现清单/风险题/抽样明细）；
- 报告须含 schema 可校验（`review_report.schema.json`），验收门禁含 `jsonschema` 校验。
...
**文档版本**：v1.2.0　**最后更新**：2026-08-19（新增 §6 评审实践模式：四层模型/抽样复审/阈值参数化/性能分桶/报告脱敏/回归固化，沉淀自 quiz-extraction-skill 评审增强实践）
