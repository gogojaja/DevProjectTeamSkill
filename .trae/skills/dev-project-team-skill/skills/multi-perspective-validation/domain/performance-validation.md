...
| 类别 | 指标 | 阈值示例 | 工具 |
|------|------|----------|------|
| 基准测试 | 单接口延迟/吞吐 | P99<200ms QPS>1000 | k6/locust/wrk/benchstat |
| 负载测试 | 持续负载下表现 | 目标 QPS 稳定运行 10min | k6/locust |
| 压力测试 | 破坏点/优雅降级 | 2x 目标负载不崩溃 | k6/locust |
| 并发测试 | 并发用户/连接/锁争用 | 无死锁/超时<5% | k6/自定义 |
| 资源利用 | CPU/内存/磁盘/网络/GC | CPU<70% 内存<80% 无泄漏 | prometheus/grafana/pprof |
| 数据库性能 | 查询延迟/连接池/锁等待 | P99<50ms 无慢查询 | EXPLAIN/pg_stat/pt-query-digest |
| 缓存命中 | 命中率/淘汰/一致性 | 命中率>95% | redis-cli/memcached-stats |
| 回归检测 | 版本间性能对比 | 无显著退化 (>10%) | benchstat/自定义对比 |
...
def run_load_test(scenario: LoadScenario) -> CheckResult:
    ...
    # 稳定性：错误率不应随时间增长
    if result.error_rate_trend > 0.01:  # 每分钟错误率增长 >1%
    ...
    # 资源稳定
    if result.memory_growth > 100_000_000:  # 100MB/小时
...
```json
{
  ...
  "checks": [
    {"id": "PERF-001", "name": "基准测试", "status": "PASS", "evidence": ["P99=45ms QPS=1200"], "severity": "high"},
    {"id": "PERF-002", "name": "负载测试", "status": "FAIL", "evidence": ["Memory growth: 150MB/h"], "severity": "high"},
    {"id": "PERF-003", "name": "压力测试", "status": "PASS", "evidence": ["Breaking point: 3.2x"], "severity": "high"},
    {"id": "PERF-004", "name": "并发分析", "status": "PASS", "evidence": [], "severity": "high"},
    {"id": "PERF-005", "name": "资源利用", "status": "FAIL", "evidence": ["GC pause P99=150ms"], "severity": "high"},
    {"id": "PERF-006", "name": "数据库性能", "status": "PASS", "evidence": [], "severity": "high"},
    {"id": "PERF-007", "name": "缓存效能", "status": "PASS", "evidence": [], "severity": "medium"},
    {"id": "PERF-008", "name": "回归检测", "status": "FAIL", "evidence": ["/api/users: P99 regressed 15%"], "severity": "high"}
  ],
  "summary": "发现内存增长疑似泄漏、GC 停顿超标、/api/orders 接口性能回归 15%",
  "confidence": "high",
  "tokens_used": 2000
}
```
...
| 指标 | 阈值 | 严重性 | 门禁 |
|------|------|--------|------|
| P99 延迟 | <200ms (API) / <50ms (DB) | high | 阻断 |
| 错误率 | <0.1% | high | 阻断 |
| 内存增长 | <50MB/h | high | 阻断 |
| GC 停顿 P99 | <100ms | high | 阻断 |
| 缓存命中率 | >95% | medium | 警告 |
| 回归阈值 | >10% 退化 | high | 阻断 |
