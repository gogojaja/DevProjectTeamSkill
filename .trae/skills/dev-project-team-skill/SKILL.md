...
description: "用户提到启用全生命周期/启用某角色/切换角色/多角色联合/项目管理模式/项目管控/阶段评审/基线固化/交接文档/角色编排/技能调度时加载本软件研发多角色编排器：按阶段渐进加载角色包（启动/需求/架构/开发/测试/投产/总控/项目群/管理咨询/项目经理执行层），支持跨模型/跨会话切换、全生命周期管控、台账评审门禁、基线固化与优先级仲裁。不加载：单一角色任务（→直接加载对应 role-*）、纯技能维护（→role-governance skill-authoring）。"
---
...
- **版本发布日期**：2026-09-09
- **版本变更记录**：
  - v21.24.1：Windows 兼容性双修复（2026-09-09）——【一】`tools/solidify.sh` v21.7.0→v21.7.2：① 技能版本解析改走 Python（新增 `read_skill_version()`），根因是 LANG 为空即 C locale 时 grep 按字节处理多字节字符，字符类 `[：:]` 中的全角冒号（UTF-8 三字节 EF BC 9A）被拆成独立字节、只能匹配 1 字节而必然失配，而纯中文字面量按字节序列整体比对仍可匹配——故症状为「`grep -c "技能版本"` 能过、含 `[：:]` 的组合正则全挂」，已用四组对照实测确认；② `MAIN_VER` 解析失败改为**报错中止**而非静默兜底 `v21.0.0`（原兜底会误建污染快照目录 `skills_backup_v21.0.0`，本次已清除该 540 文件污染目录）；③ `zip` 缺失时回退 `package_skills.py`（Git for Windows 不带 zip.exe，原致 `[4/6]` 中止并经 `set -e`+`pipefail` 连带跳过 `[5/6]` 部署；注意 package_skills.py 不接受 `--handoff`，未知参数即 exit 1，回退调用必去掉该参）；④ 9 处 `python3` 硬编码统一为探测所得 `$PY`（兼容仅有 python 的环境）。重跑验证：13 个角色包版本全部正确显示（不再 `v?`）、快照名 v21.24.0 正确、打包回退生效、部署 21 包完成、6 门禁全通过。【二】`tools/mcp_server/skills_mcp_server.py`：13 处 `subprocess.run` 收敛为 `_run()` helper 并显式 `encoding="utf-8", errors="replace"`，查明 **v21.23.1 未触及的真根因**——Windows 中文系统下 `text=True` 未指定 encoding 即按 locale(GBK) 解码，而被包装的门禁脚本内部执行 `sys.stdout.reconfigure(encoding='utf-8')` 后吐 UTF-8 字节，GBK 解码失败发生在 subprocess 的 `_readerthread` 线程内 → buffer 未 append → `communicate()` 返回 None → `r.stdout is None`；因此 v21.23.1 的 `(r.stdout or "")` **仅消除了 TypeError，门禁输出仍全部丢失**（tool 返回空串，调用方看不到任何门禁结果）。修复后实测 `check_skill_closure` 输出 1802 字符完整捕获，`✓`(U+2713) 与中文正确解码、零 U+FFFD 替换字符。附带 `stdin=subprocess.DEVNULL`：本 server 以 stdio transport 运行（文末 `mcp.run()`），子进程继承 stdin 即继承 MCP 的 JSON-RPC 通道，显式切断。【验证】`tools/tests/test_mcp_server_{stdio,expansion}.py` 12 passed；1 项 `test_02_tool_threshold_15` 失败为**存量过期断言**（期望 `MIN_TOOLS_DEFAULT = 15`，而 `check_mcp_server.py` 实际已随 commit 87787ac 升至 24，本次未触碰该文件）。【遗留】MCP server 为长驻进程，本次修复需**重启客户端 MCP 连接**方生效；另 `run_gate` 把 `skill` 作为位置参数传给门禁脚本，而 `check_skill_links.py` 用 argparse 仅接受 `--strict/--quiet`，传位置参数会 exit 2（gate=links 路径待后续统一）。
  - v21.24.0：description 跨模型触发能力重构 + 门禁标准修订（2026-09-09）——【本体】22 个技能（编排器 + 12 角色包 + 9 独立技能）frontmatter description 重构：中文触发词由 3~5 个扩充至 15~18 个（覆盖同义表述与上下位概念），新增「不加载：X（→skill-x）」反例排除段压制误触发（重叠对必填：governance↔project-mgmt↔program-mgmt、development↔architecture、testing↔development、schedule-cost↔risk-mgmt↔resource-ops）。【根因】宿主采用**全量注入 description + LLM 语义判断**（非 embedding/关键词检索），故收益来自中文触发词密度与反例排除，而非双语堆砌；已剥离试验性英文关键词括注 5949 字符（约 1500 常驻 token/会话）——强模型跨语言语义对齐本身无损（边际收益≈0），弱模型下英文反而挤占注意力预算为负收益。【配套】token_standard §2/§2.1 修订（v21.1.1→v21.2.0：长度上限 250→450、触发词密度 15~18 个、结构模板更新、新增「禁止英文关键词括注」与「反例排除（不加载段）」两条、新增「门禁覆盖口径」）；`tools/check_skill_descriptions.py` 同步（MAX_LEN 250→450、新增 EN_LIST_RE 英文关键词括注检测使文档与实现一致、find_skill_files 由仅 glob `role-*`+编排器+子技能（23 个）改为通配全部顶层技能（32 个），修复 9 个独立技能从未被检查却仍报「全部合规」的**虚假绿灯**缺陷）。【验证】双门禁全绿：check_skill_descriptions 32/32 通过、check_skill_release_gate 全角色通过；22 文件均为单行 diff（无换行符重写），剥离脚本幂等且不误伤 `(EVM)`/`(CMP/CI/config_audit)` 等既有术语块。
  - v21.23.1：MCP 服务器 subprocess 返回值防御补丁（2026-09-09）——修复 `skills_mcp_server.py` 13 处 `(r.stdout + r.stderr)[:4000]` 在 stdout/stderr 为 None 时抛 `TypeError: unsupported operand type(s) for +: 'NoneType' and 'str'` 的缺陷（首发症状：`run_gate gate=version` 报错，同类风险遍布 13 个 MCP 工具：run_gate/estimate_cost/solidify/publish_production/mirror_push/skill_links/scope_metrics/retro_harvest/review_execute/nightly_gate/raid_mgmt/evm_analyze/risk_scan）；统一改为 `((r.stdout or "") + (r.stderr or ""))[:4000]` 短路防御，最小侵入、不重构、不改架构；根因：subprocess.run 在脚本无法启动或编码异常等边界条件下 stdout/stderr 可能返回 None，原代码未做防御。修复经 role-development §4.3 根因思维（识别 13 处同类而非单点）+ §4.4 诚实汇报（症状/根因/影响面全披露）。
...
- **版本变更记录**：
  - v21.12.0：新增「任务输出模型推荐」强制规则（2026-09-01）——§5 调度新增规则 8：每条任务输出最后一行必须附带 `📦 推荐模型` 行（格式/判定/例外见 `references/model_selection.md` §5）；使模型推荐从手动行为固化为技能标准，所有项目可统一执行。
...
1. 会话启动第一步 = **「架构决策 + 环境事实校准」**：①先读项目根 `交接文档.md` 的「工作断点」区，定位上一模型已完成/待办（未读前禁止读其他项目文档，token_standard §5）；②读 `架构资产/*/ADR`（架构决策记录），校准本轮架构约束与既有决策；③**校验本机运行态与文档一致性**（已安装工具/守护服务/全局配置/台账登记），以实际运行态为锚，防止过时文档误导（本次核心教训：文档漂移）；④若 `架构资产/*/ADR` 存在 status=废弃 项，按 §2.2-7「废弃清理门禁」先行废弃资产完整性检查与清理，未清完不进入本轮任务；
...
1. 输出技术方案固定双栏模板：✅可稳定达成效果 / ⚠️理论最优效果与当前限制；要求提供方案/选型且需行业依据时，走 `skills/best-practice-solution` 水线——每条结论绑定可分级来源（T1 官方文档/RFC、T2 厂商文档/权威基准、T3 博客社区）或显式「INSUFFICIENT」弃权，标注置信度与访问日期，不得凭记忆无源直答；
...
9. **「大批量任务成本预警」铁律**：当任务预估达到以下任一阈值——①生成/修改文件 **>20 个**；②单次输出 **>50K token**；③生成大文档 **>5K 行**；④将触发多轮 agent 自循环（全库扫描 / 批量迁移 / 大批量代码生成）——执行前**必须先提示用户估算成本并给出三选一**：**A. 只制定方案与计划**（不执行生成）；**B. 分步骤执行**（分批、每步确认、可回滚，推荐本地/低价档）；**C. 指定平台/模型继续**（依 `references/dev_platform_catalog.md` 推最便宜可行档）。预警须写入 `台账/40_大模型成本台账.csv` 并触发 `select_model`（既有 action，路由归 `references/model_selection.md` 的 S0–S3）；**未获用户确认不得直接执行大批量生成**。估算动作可调用 `tools/estimate_cost.py --model <模型> --in-tok <n> --out-tok <n> --append` 计算并落台账（价格取自 `dev_platform_catalog.md` §2）。阈值可按实际账单在 `dev_platform_catalog.md` 校准（初值见上）。
10. **「执行合同闸门」铁律（防擅作主张，对齐 AGENTS.md #15）**：任何带副作用操作（不含 Tier1 纯读）执行前必须走「六段执行合同」——理解→拆解→清单→确认→执行→复验；确认段只接受白名单确认词（`执行/OK/确认/改吧/方案X`），**未确认前什么都不做**。工具风险分级：Tier1 只读自由 / Tier2 范围写（清单+确认）/ Tier3 高辐射（`~/.config/*`、外部文件写、删除、推送、`solidify`/`publish_production`/`mirror_push` 须 清单+参数哈希+备份+授权+确认词+审计台账 6 件套）。审批须绑定**参数哈希**（`tools/audit.py --param-hash` 算操作名+目标路径+改动内容 SHA 前10位），参数变更旧哈希作废须重确认；**连续 2 次红灯越界 → 全闸模式**（下一轮所有 Tier2/Tier3 走完整 6 段 + 额外一轮确认）。本合同为铁律 #7 的执行前前置强化，单一事实来源 `docs/AI助手行为门禁与授权合同.md`。
11. **「顺带操作禁令」铁律（E-01 教训）**：用户指令未显式提及的操作项，**禁止顺带执行**——即使同路径（AUTH 已授权）或上轮遗留任务，也须列入本轮清单并获确认词；把"同路径授权过"误判为"本次也授权做"即属越界反弹（红灯）。
12. **「同质操作熔断与不胜任检测」铁律（防碎片提交循环，对齐 AGENTS.md #16）**：当某工具/客户端（如 TRAE）反复执行同质化无实质新进展操作（固化/提交/审计留痕/交接断点刷新/推送重试等小改动完整闭环）时——①L1 提交批量化：同质小改动应合并提交，禁止碎片化单独立提交（`tools/commit_batch_check.py`，`--gate` 硬阻断）；②L2 固化频次提示：1 小时内固化/审计 >3 次且同质 → 提示攒批（`--freq-scan`）；③L3 不胜任判定：同质操作密度 ≥阈值（缺省 5 次/会话，可调）且无实质新进展 → 判定不胜任；有实质进展（评审/方案/落地/复盘）则胜任不误伤（`tools/incompetence_detector.py`）；④L4 交接熔断：触发 L3 且多次重复 → 立即停止当前工作（不再固化/提交/重试）→ 开始交接（交接文档断点 + 13 审计）→ 推荐替代工具/模型（按 `references/dev_platform_catalog.md` 矩阵）。违规表现：碎片提交循环反复、固化连刷断点、推送失败不停重试。细则见 `docs/工具不胜任熔断方案.md`。
13. **「目标驱动自主执行」铁律（减少交互，对齐 OPT-GOAL-001，v1.1 补强）**：用户下达任务时可附带完成目标（结构化目标定义 SGD），AI 围绕目标自主推进——①SGD 四要素：`statement`（一句话目标）+ `acceptance_criteria`（可验证验收标准列表）+ `constraints`（操作边界，v1.1 可机器验证）+ `max_iterations`（最大轮次，默认 10）；②分级交互：Tier 1 只读自由 / Tier 2 范围写（scope.write_files/write_dirs 锁定范围）**一次性批量确认** / Tier 3 高辐射仍逐项确认；③完成度自检：每步执行后对照 AC 逐项验证，全达标则退出循环进入交付；④安全护栏：max_iterations 防无限循环 + scope 读写分离锁定 + constraints 可机器验证 + 错误恢复策略（on_circuit_break/on_max_iterations）+ SGD 持久化台账（`台账/41_活跃目标.json`）+ 目标变更机制（amend）+ 铁律 #7/#15 不降级。详见 `domain/goal-driven-execution.md` + `docs/goal_driven_execution_plan.md`。
...
| **目标驱动自主执行**（OPT-GOAL-001 v1.1） | 用户下达任务时附带完成目标 / 「自主执行」「目标驱动」「不用每步确认」 | SGD 解析→回显确认→持久化台账→执行计划→一次性确认→自主执行循环（Tier 1/2 自由 Tier 3 逐项）→完成度自检→一次性交付；v1.1 增加 scope 读写分离/constraints 可验证/错误恢复/目标变更/SGD 持久化；铁律 #15 不降级，详见 `domain/goal-driven-execution.md` |
...
8. **任务输出模型推荐（强制）**：每条任务输出的最后一行必须附带推荐行（格式：`📦 推荐模型：[模型名]（[档位]，[成本]）`），按 `../references/model_selection.md` §4.2 路由决策表 + §5 输出规范判定；纯只读查询（Tier1）可省略；无推荐行 = 格式缺陷，纳入闭环执行系统验收门禁。
...
13. **目标驱动自主执行·分级交互**：在「目标驱动自主执行」模式下——①用户确认 SGD（目标+验收标准+约束）后，Tier 2 操作（目标范围内的项目内文件创建/修改）**不再逐步确认**，AI 自主推进；②Tier 3 操作（推送/删除/全局配置/solidify）仍走铁律 #15 六段合同逐项确认；③每步执行后运行 `tools/goal_check.py --goal <SGD文件>` 自检完成度；④全部 AC 达标或达到 max_iterations 时退出循环；⑤超出 SGD constraints 范围的操作立即暂停等确认。
...
**文档版本**：v21.24.1 **最后更新**：2026-09-09（Windows 兼容性双修复：① `solidify.sh` v21.7.2——版本解析改走 Python（C locale 下 grep 对多字节字符类 `[：:]` 失配，全角冒号 3 字节被拆散；纯中文字面量仍可匹配，故症状是「中文能过、组合正则全挂」）、MAIN_VER 解析失败改为中止而非静默兜底 v21.0.0（原兜底误建污染快照目录，已清除）、zip 缺失回退 package_skills.py（Git for Windows 无 zip.exe，原致 [4/6] 中止并连带跳过 [5/6] 部署）、9 处 python3 硬编码统一为 $PY；② `skills_mcp_server.py`——13 处 subprocess.run 收敛为 `_run()` 并显式 encoding="utf-8"+errors="replace"，查明 v21.23.1 未触及的真根因：text=True 未指定 encoding 时按 GBK 解码而门禁脚本吐 UTF-8，解码异常发生在 _readerthread 线程内致 communicate() 返回 None，故 v21.23.1 的 `(r.stdout or "")` 只消除 TypeError 而门禁输出仍全丢；修复后 1802 字符完整无损捕获，附 stdin=DEVNULL 切断子进程对 stdio transport 下 JSON-RPC 通道的继承；MCP 测试 12 passed（1 项存量过期断言 test_02_tool_threshold_15 与本次无关），MCP server 为长驻进程需重启客户端连接方生效；此前 v21.24.0：description 跨模型触发能力重构 + 门禁标准修订：22 个技能（编排器+12 角色包+9 独立技能）description 中文触发词 3~5→15~18 个、新增「不加载：X（→skill-x）」反例排除段压制误触发；根因为宿主是全量注入 description + LLM 语义判断（非检索），收益来自中文触发词密度与反例排除而非双语堆砌，已剥离试验性英文括注 5949 字符（约 1500 常驻 token/会话）；配套 token_standard v21.1.1→v21.2.0（长度上限 250→450、新增禁止英文关键词括注与反例排除两条、新增门禁覆盖口径）与 check_skill_descriptions.py（MAX_LEN 450、EN_LIST_RE 检测、find_skill_files 覆盖 23→32 个修复独立技能虚假绿灯）；双门禁全绿（descriptions 32/32、release_gate 全角色）；此前 v21.23.1：MCP 服务器 subprocess 返回值防御补丁：修复 `skills_mcp_server.py` 13 处 `(r.stdout + r.stderr)[:4000]` 在 stdout/stderr 为 None 时抛 `TypeError: unsupported operand type(s) for +: 'NoneType' and 'str'` 的缺陷（首发症状：`run_gate gate=version` 报错），统一改为 `((r.stdout or "") + (r.stderr or ""))[:4000]` 短路防御，最小侵入不重构不改架构，影响 13 个 MCP 工具：run_gate/estimate_cost/solidify/publish_production/mirror_push/skill_links/scope_metrics/retro_harvest/review_execute/nightly_gate/raid_mgmt/evm_analyze/risk_scan；此前 v21.23.0：协调官上下文防爆约束：§2.4.5 Executor 行明确独立子上下文执行+新增防爆约束段，禁止多 Executor 角色完整 domain 聚合单窗口，串行遵 §5 规则4/5、真并行委派 subagent/Worker、协调官只持元数据+验收结果，依据 token_standard §1.3，纳入 Verifier 门禁，回应用户 Q2；此前 v21.22.0：P2 收官（提案卡 P-003）：新增 §2.4.6 打断-修正-重规划-沉淀闭环，五步默认路径按引用串联 goal_check amend/41_活跃目标.json/dependency-graph §4+§6/deviation-detection §5.1/lesson-harvesting/interaction-harvest 六机制，补齐诉求 #7，成熟度 L4.7→L5.0；此前 v21.21.0：P1 子技能增强（提案卡 P-002）：team-orchestration v1.3.0 dependency-graph.md §6 抽象角色→13 角色包绑定表+resolve_role()（补齐诉求#2），self-improve v1.2.0 新增 interaction-harvest.md 交互自动收割（补齐诉求#6），成熟度 L4.3→L4.7；此前 v21.20.0：新增 §2.4.5 任务协调官默认流程：复杂度阈值触发的 Planner→Dispatcher→Executor→Verifier→验收闭环，绑定现有角色包、验收复用 role-governance 门禁、无新角色；提案卡 docs/任务协调官默认闭环_提案卡P-001_v1.0.md；此前 v21.19.1：goal_check.py command_pass shell 安全护栏：保留 shell=True 完整命令语义 + DANGEROUS_COMMAND_PATTERNS 12 类破坏性命令前置拦截 + _match_dangerous_command 命中即 FAIL + nosec B602 抑制 agent-loop 自主改写 + 4 单测 50 passed，修复 6758fd9 的 2 个失败测试；此前 v21.19.0（2026-09-09）：全角色自动调起机制，新增 role-operations（运维/SRE）+ role-security（安全工程）2 角色包，角色总数 10→12；iron_rules §10 升级为「角色加载铁律」；skill-loader v1.1 新增优先级 0 任务语义自动识别；role-development §4 开发人员基本素养铁律；修复 7 个 tools/ 脚本 ALL_ROLES 漏新角色 + mcp_server 路由提示词升级）
