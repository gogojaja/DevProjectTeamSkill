...
- **版本发布日期**：2026-09-08
- **版本变更记录**：
  - v21.15.0：新增「泄密文件禁止入库」最高级别铁律（2026-09-08）——`references/iron_rules.md` §3 新增 §3.2 泄密文件禁令，**凌驾于 A/B/C 三级判定，且不因仓库私有或远端仅内网而豁免**。①适用对象五类：反向映射/对照表（列为最高危，可反向破解仓库内全部脱敏成果）、未脱敏原件、凭据载体、环境情报集、人员隐私；②强制动作五项：`.gitignore` 硬屏蔽并用 `git check-ignore -v` 实测命中（仅“没暂存”不算合规）、暂存后回看变更集命中即整批退回、已推送须改写提交并强推抹除（**追加“删除提交”不算整改**，旧提交仍可 `git show <sha>:<路径>` 取出）、**排除前必须提示用户将文件手动另存到仓库外**、禁止以“脱敏后入库”变通收录映射表；③§3.2.3 划定边界：项目 `AGENTS.md` 声明的私有仓库例外仅覆盖 B 级环境信息，不覆盖本禁令；④以 `references/` 为基准重建 `shared/references/` 与部署三目录副本，8 份零漂移。触发场景：某银行项目仓库的脱敏过程记录目录含「原始词→脱敏代号」反查表且已随提交入库，据此固化本铁律。
...
**文档版本**：v21.15.0 **最后更新**：2026-09-08（新增 §3.2 泄密文件禁止入库最高级别铁律：凌驾 A/B/C 分级、不因私有或内网远端豁免、已推送须改写历史强推抹除、排除前提示用户另存；此前：v21.14.0「目标驱动自主执行」v1.1 补强）
