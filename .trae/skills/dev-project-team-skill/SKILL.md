...
13. **「目标驱动自主执行」铁律（减少交互，对齐 OPT-GOAL-001）**：用户下达任务时可附带完成目标（结构化目标定义 SGD），AI 围绕目标自主推进——①SGD 四要素：`statement`（一句话目标）+ `acceptance_criteria`（可验证验收标准列表）+ `constraints`（操作边界）+ `max_iterations`（最大轮次，默认 10）；②分级交互：Tier 1 只读自由 / Tier 2 范围写（目标相关项目内文件）**一次性批量确认** / Tier 3 高辐射仍逐项确认；③完成度自检：每步执行后对照 AC 逐项验证，全达标则退出循环进入交付；④安全护栏：max_iterations 防无限循环 + 范围锁定（超出 SGD 范围暂停等确认）+ 连续 2 步失败暂停报告 + 铁律 #7/#15 不降级。详见 `domain/goal-driven-execution.md` + `docs/goal_driven_execution_plan.md`。
...
| **目标驱动自主执行**（OPT-GOAL-001） | 用户下达任务时附带完成目标 / 「自主执行」「目标驱动」「不用每步确认」 | SGD 解析→执行计划→一次性确认→自主执行循环（Tier 1/2 自由 Tier 3 逐项）→完成度自检→一次性交付；铁律 #15 不降级，详见 `domain/goal-driven-execution.md` |
...
**文档版本**：v21.13.0 **最后更新**：2026-09-07（新增「目标驱动自主执行」模式 OPT-GOAL-001；此前：v21.12.0 任务输出模型推荐）
