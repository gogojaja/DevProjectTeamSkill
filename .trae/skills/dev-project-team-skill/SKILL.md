...
5. **双平台兼容（强制）**：本技能库所有启用项目**必须同时支持 Windows 与 macOS 双平台**——①路径/脚本/配置禁止硬编码单一平台绝对路径（如 `C:\...`、`/Users/...`），统一用相对路径或平台自适应（`os.path`/`$HOME`/`%USERPROFILE%`）；②交付物（脚本/批处理/命令）须双平台可执行，禁止依赖 Windows 专属命令（`py -3.11` 等）或 macOS 专属假设；③文本文件行尾统一 LF（配 `.gitattributes` 防 CRLF 反复 diff）；④文档中涉及路径时同时标注双平台写法；细则见 `../references/token_standard.md` §6。
