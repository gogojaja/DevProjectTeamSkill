...
description: "用户提到投产策略/发布计划/发布准备/部署执行/发布监控/回滚/回滚演练/部署脚本/Go-Live/运维交接/蓝绿部署/灰度发布/CI/CD流水线/部署自动化时加载本投产发布角色包：负责部署策略与风险分析、发布计划、准备与演练、分阶段发布执行与监控、回滚、Go-Live 评审与运维交接，输出投产方案、部署清单、执行记录与回滚预案。不加载：开发编码（→role-development）、测试执行（→role-testing）、运维监控（→role-operations）。"
---
