...
description: "用户提到台账/阶段评审/门禁/变更审计/EVM/进度/风险/安全审计/基线固化/配置管理/配置审计/归档/交接文档/文档管理/质量门禁/合规审计/治理框架时加载本总控保障角色包：负责台账与基线初始化、五维评审与门禁、变更审计、进度成本(EVM)、风险扫描、安全审计、文档管理、配置管理聚合(CMP/CI/config_audit)、基线固化/归档/交接，输出评审报告、门禁记录、审计台账与基线。不加载：日常项目管理（→role-project-mgmt）、具体技术实现（→对应 role-* 技术角色）。"
---
...
| 范围门禁/跟踪 | scope_gate | `domain/scope-change.md`（瘦引用→独立可部署技能 `scope-tracking`；工具 `tools/scope_tracker.py` v1.2.0，标准 `references/traceability_standard.md` v1.2.0） |
