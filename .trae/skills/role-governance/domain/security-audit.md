...
| **系统/项目外铁律** | 修改、删除**系统文件（%windir%\System32、hosts、注册表等）或项目外部文件（仓库之外路径，含其他项目目录）必须：①先获用户明确授权；②备份到项目内 `.backup/`（含时间戳）；③留痕至 `13_安全审计台账.csv`；否则禁止执行，经 `security_audit` 前置审计；授权经 `register_auth` 登记，**未填有效期默认仅本次对话有效** |
