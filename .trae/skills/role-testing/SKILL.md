---
name: "role-testing"
description: "用户提到测试策略/测试计划/用例设计/测试环境/测试数据/执行测试/缺陷管理/测试总结/回归测试/性能测试/安全测试/验收测试/测试覆盖率/测试门禁时加载本测试管理角色包：负责可测性分析与风险测试策略、测试计划、用例设计（等价类/边界/决策表/状态迁移）、环境数据准备、执行与缺陷管理、总结与门禁退出，输出测试追溯矩阵、缺陷清单与门禁结论。不加载：需求分析（→role-requirements-analysis）、代码走查（→role-development）、安全审计（→role-security）。"
---

# role-testing 测试管理角色包

> 版权：`../shared/references/COPYRIGHT.md`　Token：`../shared/references/token_standard.md`　总控：`../shared/governance.md`

## 1. 元数据

- **技能版本**：v21.0.0　**发布日期**：2026-08-04
- **变更记录**：v21.0.0 由 test-management-skill + 6 子技能重组为角色包
- **参考标准**：ISTQB · ISO/IEC/IEEE 29119 · ISO/IEC/IEEE 29119-4 用例设计

## 2. 触发规则

用户表达「测试策略/测试计划/用例设计/环境准备/执行测试/缺陷/测试总结/测试报告」时加载本包。先 Read 路由表，命中后只读对应 `domain/*.md`。

## 3. 流程（路由到 domain/）

| 环节 | action | 明细 |
|------|--------|------|
| 策略分析 | analyze_testability / risk | `domain/strategy.md`（RBT/方法/指标） |
| 计划编写 | write_plan | `domain/planning.md`（入口/出口/资源/环境/CI） |
| 用例设计 | design_cases | `domain/design.md`（等价类/边界/决策表/状态迁移/RTM） |
| 环境数据 | prepare_env | `domain/preparation.md`（环境/数据/Mock/配置/冒烟） |
| 执行缺陷 | execute_test / manage_defect | `domain/execution.md`（风险序/探索性/缺陷生命周期） |
| 总结门禁 | write_report / stage_review | `domain/summary.md`（对接总控） |
| Excel 需求评审（S-01） | xlsx_review | `domain/xlsx_review.md`（T-01→规则检查→T-03→T-04 端到端） |
| AI 代码审查（S-04） | code_review | `domain/code_review.md`（T-08 多层分析→脱敏→门禁） |
| Excel 自动修复（S-02） | xlsx_auto_fix | `domain/xlsx_auto_fix.md`（T-05→T-06→T-04 修复闭环） |
| 安全扫描套件（S-05） | security_scan | `domain/security_scan.md`（T-09+脱敏+审计→聚合报告） |
| 评审复查（S-06） | review_recheck | `domain/review_recheck.md`（T-06→T-03→T-04 复查闭环） |
| Word 需求评审（S-03） | docx_review | `domain/docx_review.md`（T-07→IEEE830→T-03→T-04） |

## 4. 测试铁律

风险驱动执行排序；缺陷管理 New→Closed 生命周期；用例须 IEEE 830 RTM 追溯；DoD 与报告按 token_standard §3 输出 CSV。

## 5. 输出规范与边界

- 用例 / 缺陷 / 报告按 token_standard §3 输出 CSV；
- 阶段评审退出门禁经 `../shared/governance.md`；
- 边界：仅测试域；需求链路回溯到 role-requirements-analysis。

---

## 闭环执行系统

### 1. 任务入口
- 输入：用户要求测试策略、设计用例、执行测试、管理缺陷或输出测试总结；
- 前置：需基于需求/架构/实现状态确认测试范围、环境和准入条件；
- 不适用：不具备版本、用例、环境或测试对象时，不应误判执行状态。

### 2. 执行状态
| 状态 | 进入条件 | 退出条件 | 处理方式 |
|------|---------|---------|---------|
| 待启动 | 测试对象与范围已明确 | 测试计划或用例启动 | 读取需求和实现状态 |
| 执行中 | 用例执行已开始 | 关键缺陷或结论形成 | 按风险优先级推进 |
| 校验中 | 测试和缺陷录入已完成 | 结果是否通过被确认 | 检查缺陷闭环与报告完整性 |
| 阻塞 | 环境不稳定、数据异常、缺陷未确认 | 补充环境/确认缺陷 | 暂停并记录问题 |
| 完成 | 测试报告和门禁通过 | 进入交接或退出阶段 | 归档证据并交接 |
| 回退 | 关键缺陷未修复或脚本环境失败 | 回到最近稳定状态 | 复盘失败原因并修正用例/环境 |

### 3. 执行动作层
- 执行步骤 1：分析可测性与风险，编写测试计划；
- 执行步骤 2：设计测试用例并准备环境和测试数据；
- 执行步骤 3：执行测试、收集缺陷并撰写总结门禁；
- 所需工具/脚本：测试计划模板、用例设计矩阵、缺陷管理流程、总控台账；
- 输入输出约束：测试结果必须具备证据和缺陷闭环，而非单纯完成执行。

### 4. 验收门禁
- 必须产出物：测试计划、用例清单、缺陷清单、测试报告；
- 通过条件：关键用例执行完成、严重缺陷已关闭或被允许，门禁通过；
- 失败条件：关键需求未覆盖、严重缺陷未处理、环境不稳定导致结果失真；
- 审核对象：测试负责人 + role-governance 总控。

### 5. 失败处理
- 失败类型：用例失败、环境异常、缺陷数据不完整、测试设备问题；
- 恢复策略：复核测试数据、环境与脚本，最小范围重跑；
- 回滚方案：回到稳定环境状态，保留失败证据；
- 重试策略：在环境与用例修正后重试，避免无效重复执行；
- 是否需要人工确认：重大缺陷、回归放行、发布门禁需人工确认。

### 6. 产出与交接
- 产出物列表：测试计划、用例、缺陷清单、测试总结/报告；
- 保存路径：项目测试目录、总控台账和交接文档；
- 交接对象：开发、部署、项目负责人和总控；
- 下一步动作：修复缺陷、进入投产门禁或标记阶段结束；
- 归档条件：测试门禁通过并保留完整证据链。

### 7. 审计记录
- 执行时间：测试计划、执行、缺陷处理和总结时间；
- 关键参数：用例 ID、环境版本、缺陷等级、版本号；
- 关键决策：是否通过、是否阻塞、是否回归；
- 结果证据：测试日志、缺陷系统、CSV 报告；
- 失败原因：记录在总控审计台账和交接文档中。

---

**文档版本**：v21.0.0　**最后更新**：2026-08-04
**知识产权所有**：段波（验证邮箱：duanbo.douglas@163.com）