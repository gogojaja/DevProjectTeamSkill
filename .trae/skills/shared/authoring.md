...
> 若技能仅有流程描述、最佳实践、角色职责说明而无闭环执行系统，则不视为合格产出。
...
### 1. 任务入口
- 输入：什么情况下触发
- 前置：需要什么上下文、文件、台账、角色状态
- 不适用：什么情况下不该执行

### 2. 执行状态
| 状态 | 进入条件 | 退出条件 | 处理方式 |
|------|---------|---------|---------|
| 待启动 | 触发条件满足 | 用户确认/系统启动 | 准备上下文 |
| 执行中 | 任务已启动 | 产出形成/失败 | 执行动作 |
| 校验中 | 关键动作完成 | 验收通过/失败 | 检查门禁 |
| 阻塞 | 依赖缺失/信息不足 | 补充信息/人工处理 | 暂停推进 |
| 完成 | 验收通过 | 进入交接 | 记录证据 |
| 回退 | 执行失败/门禁未过 | 回到最近稳定状态 | 启动回滚 |
