...
- **每阶段末**（`stage_review` 后、交接前）执行授权时效检查：列出 `14_授权登记.csv` 中「有效期已过/临近(≤7 天)/已撤销」的授权，提醒用户确认续期或撤销；检查结果写入阶段复盘（22_阶段复盘.csv）。

---

**文档版本**：v21.3.2　**最后更新**：2026-08-07（新增 register_auth/授权登记+阶段末时效检查）
