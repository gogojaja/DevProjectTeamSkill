...
description: "用户提到收集需求/分析需求/编写SRS/需求规格/需求评审/需求变更/需求追溯/需求基线/用户故事/用例分析/需求优先级/需求冲突解决时加载本需求分析角色包：负责需求收集、七维度分析、IEEE 830 SRS 编写、需求评审、变更影响分析与双向追溯，输出 SRS、需求收集清单、需求冲突记录、变更评估表与追溯矩阵。不加载：架构设计（→role-architecture）、测试用例设计（→role-testing）。"
---
