...
**文档版本**：v21.12.0
**最后更新**：2026-09-05（条目21 远程控制技能 v1.0.0：ADB+scrcpy集成。此前 2026-09-02：条目19 孵化器立项阶段技能 v1.1.0）
...
21. **远程控制技能（v1.0.0）**：远程控制Android设备的技能，支持ADB连接、屏幕镜像、文件传输和设备管理。通过scrcpy实现高质量的屏幕投射和控制，支持USB和无线连接方式；触发词 `远程控制`/`手机连接`/`屏幕镜像`/`ADB连接`/`scrcpy`/`设备管理`，详见 `tools/remote-control/SKILL.md`。
