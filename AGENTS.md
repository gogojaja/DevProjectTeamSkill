...
> **独立关联项目（仅登记不内嵌，单一信源）**：
> - 局域网 Git 基建由**独立项目 `dev-git-hub` 为单一信源**（授权 AUTH-014，路径经 `$DEV_GIT_HUB_ROOT` 或同级目录约定自动定位）承载——Mac 局域网 bare 中枢 + Windows 全量副本 + WAN 灾备 + git 复杂远端操作工具（mirror_push/github_push/github_ip_refresh/restore_github_push/_gh_ip_probe/github_access 标准）。**所有 git 基建方案/工具/标准以 dev-git-hub 为唯一权威（见其 交接文档.md）**；本仓库**不保留实现**，仅经 **薄封装代理调用**（tools/ 下同名代理注入 PROJECT_ROOT 指向目标仓库）+ 引用登记。接口共享：dev-git-hub 脚本以 `PROJECT_ROOT`/`DPB_ROOT` 环境变量为目标仓库工作根（读其远端/台账），本仓库代理转发时注入。避免技能库膨胀与硬件配置耦合。详见 dev-git-hub 仓库内 `交接文档.md` 与 `README.md`。
> - 定时任务管理由**独立项目 `dev-task-scheduler` 为单一信源**（授权 AUTH-015，路径经 `$DEV_TASK_SCHEDULER_ROOT` 或同级目录约定自动定位）承载——基于 APScheduler 3.11.3 的跨项目调度引擎（幂等/重试/告警/状态持久化）。**所有调度器方案/工具/标准以 dev-task-scheduler 为唯一权威（见其 README.md）**；本仓库**不保留实现**，仅经 **薄封装代理调用**（tools/scheduler_proxy.py 注入 PROJECT_ROOT 指向目标仓库）+ 引用登记。避免技能库膨胀。详见 dev-task-scheduler 仓库内 `README.md`。
> - 多模型分层编排由**独立项目 `dev-model-router` 为单一信源**（授权 AUTH-016，路径经 `$DEV_MODEL_ROUTER_ROOT` 或同级目录约定自动定位）承载——Router + DAG + Executor 三层架构（复杂度评估/模型选择/DAG 分解/分阶段执行/结果组装）。**所有编排器方案/工具/标准以 dev-model-router 为唯一权威（见其 README.md）**；本仓库**不保留实现**，仅经 **薄封装代理调用**（tools/model_router_proxy.py 注入 PROJECT_ROOT 指向目标仓库）+ 引用登记。避免技能库膨胀。详见 dev-model-router 仓库内 `README.md`。
> - 项目管理工具集由**独立项目 `dev-project-mgmt` 为单一信源**（授权 AUTH-022，路径经 `$DEV_PROJECT_MGMT_ROOT` 或同级目录约定自动定位）承载——RAID 管理/进展报告/变更协调/EVM 计算工具实现；方法论在本库 `role-project-mgmt` 技能（单一信源互补：方法在技能库，代码在独立项目）。立项 INC-2026-09-01-001（评分 94/100）。远端仓库待建。
> - 安全审计工具集由**独立项目 `dev-security-tools` 为单一信源**（授权 AUTH-023，路径经 `$DEV_SECURITY_TOOLS_ROOT` 或同级目录约定自动定位）承载——审计台账/脱敏扫描/授权边界检查/密钥泄漏检测的**新增**工具；本仓库现有 `tools/audit.py`、`tools/desensitize/` **保留原位不迁移**。立项 INC-2026-09-01-002（评分 96/100）。远端仓库待建。
> - 测试工具集由**独立项目 `dev-test-tools` 为单一信源**（授权 AUTH-024，路径经 `$DEV_TEST_TOOLS_ROOT` 或同级目录约定自动定位）承载——跨项目测试执行/覆盖率聚合/缺陷台账/报告生成工具实现；方法论在本库 `role-testing` 技能（单一信源互补）。立项 INC-2026-09-01-003（评分 95/100）。远端仓库待建。
...
## 职责边界补充（2026-09-05 项目群边界审核）

> 本节由项目群边界审核（B-0）自动写入，与上方铁律同等约束力。

### 本项目在项目群中的定位

- **类型**：技能库 + PMO（项目群管理办公室）
- **核心职责**：软件研发全生命周期多角色编排；项目群整体视图与管控（注册表/进度/依赖/标准）
- **不承载**：git 基建实现（→ dev-git-hub）、卫星工具库实现（→ 各 dev-* 项目）、双机工作站部署架构（→ TwinForge）

### 调用关系

- 薄代理转发：dev-git-hub（7 个）、dev-model-router（1 个）、dev-task-scheduler（1 个）
- 直接调用：dev-project-mgmt / dev-security-tools / dev-test-tools（角色包按需使用）
- 项目群管理：所有成员项目（registry/进度/依赖/标准统一管理）
